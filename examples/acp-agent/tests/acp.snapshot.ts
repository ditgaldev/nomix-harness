...
import { resolvePwshPath } from '@nomix-ai/nomix-pwsh-local'
import { parseSessionLog } from '@nomix-ai/nomix-llm-replay'
import { OFFLOADED_IMAGE_TEXT } from '@nomix-ai/nomix-llm'

/**
 * The acp-agent example's snapshot suite: the scenario table for
 * `nomix-acp-snapshot`'s suite factory, which owns every compare/guard mechanic
...
// The nomix-acp-demo bin (the demo:acp entry), this example's cordis.yml, and
...
async function prepareEditingCordisSkillWorkspace(cwd: string): Promise<void> {
  const target = join(cwd, '.nomix', 'skills', 'editing-cordis-compositions', 'SKILL.md')
...
function snapshotModeFromEnv(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    case 'refresh':
      return 'refresh'
    default:
      throw new Error(`unknown NOMIX_SNAPSHOT mode: ${value}`)
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  // `false` command exits 1, rather than becoming SANDBOX_UNAVAILABLE.
  {
    ...
    env: { NOMIX_PERMISSION_MODE: 'read-only' },
    posixOnly: true,
  },
  ...
  // foreground error and background job marker without a platform runner.
  {
    ...
    configPath: PARTIAL_LANDLOCK_CONFIG,
    env: {
      NOMIX_PERMISSION_MODE: 'read-only',
      NOMIX_SNAPSHOT_MISSING_SANDBOX_RUNNER: '1',
    },
    posixOnly: true,
  },
  ...
  // (XXX(concurrent-subagents) in nomix-llm-replay).
  ...
  // published-handle disposal failure.
  {
    ...
    env: { NOMIX_SUBAGENT_PUBLISHED_FAILURE: '1' },
    hasModelTurn: true,
    recorded: false,
    overridden: true,
    configPath: SUBAGENT_DURABILITY_FAILURE_CONFIG,
  ...
  // no session-scoped UI picker on the automation protocol.
  {
    ...
    systemPromptSource: 'text-turn',
    toolSchemasSource: 'text-turn',
    env: { NOMIX_PERMISSION_MODE: 'workspace-write' },
  },
  {
...
defineAcpSnapshotSuite({
  ...
  scenarios: SCENARIOS,
  mode: snapshotModeFromEnv(process.env.NOMIX_SNAPSHOT),
  hasPwsh,
})
...
it('pins native DeepSeek Files offload and inline fallback in assembled requests', async () => {
  ...
  try {
    const result = await runScenario(input, {
      ...
      env: {
        NOMIX_SNAPSHOT_API_KEY: 'snapshot-key',
        NOMIX_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
    ...
    const fallback = await runScenario(input, {
      ...
      workspaceDir: join(SNAPSHOTS_DIR, 'read-image', 'workspace'),
      env: {
        NOMIX_SNAPSHOT_API_KEY: 'snapshot-fallback-key',
        NOMIX_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })

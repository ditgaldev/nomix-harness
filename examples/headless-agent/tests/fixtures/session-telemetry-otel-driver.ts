...
import { boot, resolveConfigPath } from '@nomix-ai/nomix-app-boot'
import { recordFeedback } from '@nomix-ai/nomix-command-feedback'
import { runFixtureTurn } from '@nomix-ai/nomix-loader-smoke'
...
process.env.NOMIX_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
...
try {
  ...
  const mode = process.env.NOMIX_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {

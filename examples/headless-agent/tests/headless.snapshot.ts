...
import {
  decompressZstdFrame,
  scanZstdFrames,
} from '@nomix-ai/nomix-session-persistence-jsonl/src/zstd.ts'
...
const nomixBinScript = fileURLToPath(new URL('../../../apps/cli/src/bin.ts', import.meta.url))
...
async function prepareCliMockFixture(cwd: string): Promise<void> {
  const fixtureDir = join(cwd, '.nomix', 'profiles', 'headless', 'snapshot-fixtures')
...
describe('headless stream-json snapshots', () => {
  ...
  it('recovers from context overflow through an assembled compaction', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        NOMIX_SNAPSHOT: 'replay',
        NOMIX_SNAPSHOT_FILE: compactionSessionFixture,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('logs actionable missing-credential guidance through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tsconfigPath,
      env: {
        // First-run posture: no key in the environment, none under ./.nomix.
        DEEPSEEK_API_KEY: '',
        DEEPSEEK_BASE_URL: '',
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
    ...
    try {
      const result = await runLoaderSmoke({
        ...
        env: {
          ...
          NOMIX_SNAPSHOT_BASE_URL: server.url,
          NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
        },
      })
  ...
  it('replays the advanced toolchain through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        NOMIX_SNAPSHOT: 'replay',
        NOMIX_SNAPSHOT_FILE: advancedSessionFixture,
        NOMIX_SNAPSHOT_CHILD_FILES: [
          join(advancedScenarioDir, 'session.1.jsonl'),
          join(advancedScenarioDir, 'session.2.jsonl'),
        ].join(delimiter),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
  ...
  it('replays persisted goal tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        NOMIX_SNAPSHOT: 'replay',
        NOMIX_SNAPSHOT_FILE: join(goalScenarioDir, 'session.jsonl'),
        NOMIX_SNAPSHOT_OVERRIDE: join(goalScenarioDir, 'replay.override.json'),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('replays two fresh Ralph rounds through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        NOMIX_SNAPSHOT: 'replay',
        NOMIX_SNAPSHOT_FILE: join(ralphScenarioDir, 'session.jsonl'),
        NOMIX_SNAPSHOT_OVERRIDE: join(ralphScenarioDir, 'replay.override.json'),
        NOMIX_SNAPSHOT_CHILD_FILES: [
  ...
  it('delivers a continuable child result without parent polling', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        NOMIX_SNAPSHOT_FILE: parentReplay,
        NOMIX_SNAPSHOT_OVERRIDE: parentOverride,
  ...
  it('replays persistent PTY tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        NOMIX_SNAPSHOT: 'replay',
        NOMIX_SNAPSHOT_FILE: ptySessionFixture,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {

---
name: nomix-translate-docs
description: Manually run the extended Nomix Harness bilingual-document workflow, including generated briefings, delegated prose translation, whole-document translation, and scoped pairing verification.
disable-model-invocation: true
user-invocable: true
---

# Translating Nomix-Harness docs

## Invocation boundary

Run this extended workflow only when the user explicitly invokes `nomix-translate-docs` by name. Never select or load it for ordinary documentation work, from another skill, or from an inferred translation need; routine translation follows the one-shot, one-pass rule in [docs/AGENTS.md](../../../docs/AGENTS.md).
...
- **[nomix-prose-standard](../nomix-prose-standard/SKILL.md)** — required prose coverage and editorial judgment. Apply it to both sides without adding or dropping source propositions.
...
4. Before the PR: the touched pairs are green under the scoped check; `pnpm run doc-sync` (which includes the corpus-wide pairing check plus `verify-md-wrap`/`verify-md-links`) runs once at PR level per [nomix-pre-push-checks](../nomix-pre-push-checks/SKILL.md), not inside each translation task.

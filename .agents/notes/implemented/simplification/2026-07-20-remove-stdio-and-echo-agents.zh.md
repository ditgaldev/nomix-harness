...
- [`nomix --profile headless`](../../../../apps/cli/README.md) 负责非交互式执行。其 `headless` profile 是产品组装；`examples/headless-agent` 负责回放快照、通用真实 agent 测试套件和未导出的无密钥 Loader driver。
- [`@nomix-ai/nomix-acp-demo`](../../../../packages/examples/acp-demo/README.md) 和 `@nomix-ai/nomix-sdk-jsonrpc-server` 负责各自的分帧协议集成。

# Agent Note: Omit runtime invariants from shipped nomix config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@nomix-ai/nomix-*/invariant` entry.
...
- Ordinary `nomix` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.

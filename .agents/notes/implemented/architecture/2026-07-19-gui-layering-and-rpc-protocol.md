...
- One `dsh` process supporting both `nomix web` (serve) and `nomix --profile headless` (headless) — one process, two modes (a design reservation)
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/cli` (`@nomix-ai/nomix-harness`) dispatches commands: `nomix web` = Host + webserver + the built `dsh-web-frontend` dist; `nomix --profile headless` = [a direct core Agent/Session entry point](2026-08-09-headless-direct-core-entry-point.md), with zero Host, HTTP, or browser layer.

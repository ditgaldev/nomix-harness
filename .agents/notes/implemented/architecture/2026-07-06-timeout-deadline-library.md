...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `nomix-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `nomix-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `nomix-timeout` (`idleWatchdog`) |
| Clear the timer | `nomix-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `nomix-timeout` (`timeoutOf`) |

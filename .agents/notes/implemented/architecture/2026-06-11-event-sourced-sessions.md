...
- Persistence stays a plugin concern; the in-memory store ships in nomix-session.
...
- Derivation cost grows with log length — compaction (nomix-compaction) is the intended mitigation, not log mutation.

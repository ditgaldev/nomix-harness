...
The `pack` job runs `pnpm deploy` for the CLI with production dependencies, places that portable directory under the standard `package/` archive root, rejects any tarball member outside that root, installs the tarball into a throwaway consumer, and drives `nomix --version`. The `publish` job downloads and uploads those exact bytes without rebuilding. Internal workspace packages remain implementation units in the repository and installed package, not independent npm publications.

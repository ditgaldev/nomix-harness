...
All three publish publicly to the `@nomix-ai` scope on npmjs.com, and each manifest owns its intended access level. The Nomix workflow requires an anonymous `npm view` to succeed after publication. Its `repair_access` dispatch reapplies public access and runs the same anonymous check without rebuilding or republishing the tarball; the npm credential must be authorized to change package access, and an interactive package owner must use npmjs.com when registry policy requires a 2FA challenge.

### Versions land in the repository from a local command; CI only checks and uploads

Each sequence has one bump-and-commit command: it derives the target version, writes it into the relevant manifests, runs `pnpm install --lockfile-only`, and commits the manifests with the lockfile. The published version is therefore readable from the repository. Nomix publishes from pushes to `npm-nomix-harness`; vendor publication tags remain human-created commit pointers. CI never writes to the repository and needs no write permission.
...
|---|---|---|
| `@nomix-ai/cordis` | 4.0.0-rc.7 | 4.0.1 |
| `@nomix-ai/cordis-plugin-loader` | 1.0.0-rc.5 | 1.0.1 |
| `@nomix-ai/cosmokit` | 1.8.1 | 1.8.2 |
| `@nomix-ai/schemastery` | 3.18.0 | 3.18.1 |
| `@nomix-ai/cordis-plugin-hmr` | 1.0.15 | 1.0.16 |
| `@nomix-ai/cordis-plugin-include` | 1.0.4 | 1.0.5 |
| `@nomix-ai/cordis-plugin-timer` | 1.1.2 | 1.1.3 |
| `@nomix-ai/cordis-plugin-group` | 1.0.0 | 1.0.1 |
| `@nomix-ai/cordis-plugin-logger-console` | 1.0.0 | 1.0.1 |
...
`scripts/check-workspace-constraints.ts` requires the protocol, so a new package cannot reintroduce a hand-written range; the invariant-companion rule requires `workspace:^` for `@nomix-ai/nomix-invariants` for the same reason.
...
| `verify` | the family's version baseline, the publish order it prints in full, and — when publishing — that the run comes from an approved publication ref and its members are publishable |
...
The `pack` job runs `pnpm deploy` for the CLI with production dependencies and materializes its links. It removes the platform-selected copies of `sharp`, `koffi`, and `node-addon-require-builtin`, declares their exact versions as ordinary npm dependencies, compresses the remaining tree into one `nomix-runtime.tgz` member, and places the portable package under the standard `package/` archive root. npm therefore selects those wrappers' native optional packages for the consumer's operating system and architecture instead of receiving Linux binaries from the pack runner. The install lifecycle restores the runtime with the host `tar` command so Windows does not spend minutes creating the tree through JavaScript; it falls back to the bundled JavaScript extractor only when `tar` is unavailable. Installations that disable package scripts or optional dependencies are unsupported. Pack rejects paths outside the root, hard-link entries, a missing runtime archive, or an expanded runtime exposed as npm members before it installs the tarball once, checks `nomix --version` through the installed npm-exec bin, and loads the Web application through `nomix web --help`. Windows x64 and macOS jobs repeat that installed-artifact probe over the exact Linux-produced tarball before publication can start. The `publish` job downloads and uploads those verified bytes without rebuilding. Internal workspace packages remain implementation units in the repository and installed package, not independent npm publications.

`pack` carries no credentials and runs on every pull request, every push to `npm-nomix-harness`, and every `nomix-v*` tag push, so a pull request or version tag proves the release set still packs. `publish` runs only for `npm-nomix-harness`, sits behind the `npm-publish` environment for human approval, and neither builds nor rebuilds — it uploads the bytes pack produced. Pack runs are grouped per ref so concurrent pull requests do not displace each other; branch runs that may publish are not cancelled, and the publish job carries the global group because dist-tags are shared registry state.

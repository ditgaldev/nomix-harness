...
The `pack` job runs `pnpm deploy` for the CLI with production dependencies, places that portable directory under the standard `package/` archive root, stores hard-linked deployment files as independent archive members, rejects paths outside the root or remaining hard-link entries, installs the tarball into a throwaway consumer, and drives `nomix --version`. The `publish` job downloads and uploads those exact bytes without rebuilding. Internal workspace packages remain implementation units in the repository and installed package, not independent npm publications.

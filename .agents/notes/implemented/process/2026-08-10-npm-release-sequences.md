...
The `pack` job runs `pnpm deploy` for the CLI with production dependencies, including native optional packages for the Linux build host and Windows x64 consumer, materializes links, compresses the expanded `node_modules` tree into one `nomix-runtime.tgz` member, and places the portable package under the standard `package/` archive root. Its install lifecycle restores that runtime with a bundled extractor; installations that disable package scripts are unsupported. Pack rejects paths outside the root, hard-link entries, a missing runtime archive, or an expanded runtime exposed as npm members before it executes the tarball through `npm exec`, checks `nomix --version`, and loads the Web application through `nomix web --help`. A Windows x64 job repeats that installed-artifact probe over the exact Linux-produced tarball before publication can start. The same install path covers `npx --package @nomix-ai/nomix-harness nomix`. The `publish` job downloads and uploads those verified bytes without rebuilding. Internal workspace packages remain implementation units in the repository and installed package, not independent npm publications.

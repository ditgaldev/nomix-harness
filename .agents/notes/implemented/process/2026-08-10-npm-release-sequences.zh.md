...
`pack` job 对 CLI 及其生产依赖运行 `pnpm deploy`，把可移植目录放到标准的 `package/` 归档根下，把部署树中的硬链接文件分别写成独立的归档成员，拒绝该目录之外的路径或残留的硬链接条目，然后在一次性 consumer 中安装该 tarball，并执行 `nomix --version`。`publish` job 下载并上传完全相同的字节，不再构建。内部 workspace 包仍是仓库和安装产物中的实现单元，不再是独立 npm 发布。

...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `nomix-tool-bash` (`bash`), `nomix-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `nomix-tool-subagent` (`subagent`). Example-only tools are excluded.

...
| Diagnostic type and byte limit | `nomix-subagent` | Consumers receive a bounded optional field separate from assistant output. |
| Foreground and Job presentation | `nomix-tool-subagent` and the generic Job runtime | Scheduling choice does not change the underlying failure fact. |
| Process cancellation and quiescence | Product Provider and `nomix-subprocess` | Result settlement still precedes idempotent whole-tree disposal. |

...
- The commands package now depends on `nomix-attachment` and `nomix-llm`, and `commands/execute` carries a required `images` wire parameter — every caller states its envelope explicitly.

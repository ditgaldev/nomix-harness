...
- `@nomix-ai/nomix-session-persistence` is removed as a package.
- `nomix-session` exports the persistence service type, coordinator, and contract helpers.
- JSONL and SQLite backend packages depend on `nomix-session` directly.

...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`nomix-llm-mock-server` `pause()`, `nomix-lsp-stdio`, `nomix-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:

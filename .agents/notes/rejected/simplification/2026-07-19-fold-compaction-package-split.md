...
Compaction is split between `@nomix-ai/nomix-compaction`, which owns an abstract two-method service and shared types, and `@nomix-ai/nomix-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@nomix-ai/nomix-compaction` and remove `@nomix-ai/nomix-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@nomix-ai/nomix-compaction-basic` and its workspace/package metadata are removed.
- `@nomix-ai/nomix-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.

...
class NomixHarness:
    """Reusable synchronous SDK for running Nomix Harness agent turns.
    ...
    def __init__(self, config: NomixHarnessConfig | None = None, **kwargs: object) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either NomixHarnessConfig or keyword options, not both")
        self.config = config or NomixHarnessConfig(**kwargs)
        ...
        if self.config.session_root is not None:
            env["NOMIX_SESSION_ROOT"] = self.config.session_root
    ...
    def __enter__(self) -> "NomixHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: NomixHarness, session_id: str) -> None:

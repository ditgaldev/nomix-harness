...
const snapshotMaxConcurrency = positiveIntFromEnv(
  'NOMIX_SNAPSHOT_MAX_CONCURRENCY',
...
if (process.env.NOMIX_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.NOMIX_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.NOMIX_SNAPSHOT || 'replay') === 'replay' && snapshotMaxConcurrency > 1,

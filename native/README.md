...
The main repository's `Landlock Run` workflow builds and tests each supported architecture. `Release (Nomix)` builds both Linux binaries on native runners and embeds them in `@nomix-ai/nomix-harness`; the native workspaces are private and publish no npm packages.

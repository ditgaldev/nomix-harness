# @nomix-ai/nomix-skill-badge

English | [中文](README.zh.md)

Optional bundled skill provider that contributes `nomix-badge` to `ctx.skills`. The skill supplies the official “powered by nomix” Markdown snippets and the packaged PNG for systems that cannot import a remote image reliably.
...
The provider exposes its packaged `assets/` directory as the skill resource base. `nomix-badge.png` is the 726×120 source asset, and consumers render it at 121×20.

## Model Experience

Indirectly, through `@nomix-ai/nomix-tool-skill`, which renders the catalog entry and selected skill body.

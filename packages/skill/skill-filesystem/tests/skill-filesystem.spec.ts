...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@nomix-ai/nomix-fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `nomix-${name}-`)))
}
...
describe('nomix-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.nomix/skills'), 'same', 'user nomix skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.nomix/skills'), 'same', 'project nomix skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.nomix/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project nomix skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-nomix')
    ...
    await writeSkill(join(noGit, '.nomix/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.nomix/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.nomix/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.nomix/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.nomix/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.nomix/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.nomix/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.nomix/skills/broken-link'))
    await symlink('/dev/null', join(home, '.nomix/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { nomixHome: join(home, '.nomix'), agentsHome: join(home, '.agents'), watch: false })

    expect((await ctx.skills.list({ cwd: nestedCwd })).map(skill => [skill.name, skill.source])).toEqual([
      ['backend-root', 'project-agents'],
      ['text-skill', 'user-nomix'],
    ])
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.nomix/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.nomix/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousNomixHome = process.env.NOMIX_HOME
    const previousAgentsHome = process.env.NOMIX_AGENTS_HOME
    const previousBundledSkillDir = process.env.NOMIX_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.NOMIX_HOME = join(envHome, '.nomix')
      process.env.NOMIX_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.NOMIX_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.nomix/skills'), 'env-skill', 'Env skill')
      ...
      process.env.NOMIX_HOME = join(envHome, 'empty-nomix')
      delete process.env.NOMIX_BUNDLED_SKILL_DIR
      process.env.NOMIX_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { nomixHome: join(envHome, 'empty-nomix') }).name).toBe('filesystem')
    } finally {
      if (previousNomixHome === undefined) {
        delete process.env.NOMIX_HOME
      } else {
        process.env.NOMIX_HOME = previousNomixHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.NOMIX_AGENTS_HOME
      } else {
        process.env.NOMIX_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.NOMIX_BUNDLED_SKILL_DIR
      } else {
        process.env.NOMIX_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})

 ...
 *
 * @module @nomix-ai/nomix-agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from '@nomix-ai/nomix-fs'
import { assertNever } from '@nomix-ai/nomix-llm'
import { nomixHomeDisplay } from '@nomix-ai/nomix-home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.nomixHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.nomixHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    ...
    displayPath: directory === USER_GLOBAL_DIRECTORY
      ? userGlobalDisplayPath(resolved.nomixHome)
...
function userGlobalDisplayPath(nomixHome: string): string {
  return `${nomixHomeDisplay(nomixHome)}/AGENTS.md`
}

...
import ToolRegistry, { defineContentToolFixture } from '@nomix-ai/nomix-tools'
import { FILE_REFERENCE_PROMPT } from '@nomix-ai/nomix-file-reference'
...
async function stubAgent(
  ...
  const root = await mkdtemp(join(tmpdir(), 'nomix-file-reference-service-'))
  roots.push(root)

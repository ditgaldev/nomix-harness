...
import { CodeRuntime } from '@nomix-ai/nomix-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@nomix-ai/nomix-code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from '@nomix-ai/nomix-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, Message, StreamChunk } from '@nomix-ai/nomix-llm'
import SystemPrompt from '@nomix-ai/nomix-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@nomix-ai/nomix-tools'
import type { Config as ToolConfig } from '@nomix-ai/nomix-tools'
import LocalFileSystem from '@nomix-ai/nomix-fs-local'
import * as FsPolicy from '@nomix-ai/nomix-fs-observation-policy'
import LocalAttachmentStore from '@nomix-ai/nomix-attachment-local'
import { AttachmentError, AttachmentId, AttachmentStore } from '@nomix-ai/nomix-attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'nomix-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'nomix-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { nomixHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { nomixHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { nomixHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])

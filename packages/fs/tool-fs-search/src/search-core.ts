 ...
 *
 * @module @nomix-ai/nomix-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@nomix-ai/nomix-output-retention'
import type { RetainedItems } from '@nomix-ai/nomix-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@nomix-ai/nomix-subprocess'
import type { SaveTextSpill, SpillRef } from '@nomix-ai/nomix-spill'
 ...
 * `@nomix-ai/nomix-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`nomix-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@nomix-ai/nomix-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@nomix-ai/nomix-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@nomix-ai/nomix-tool-fs-search/presentation} `grepSearchMeta`)

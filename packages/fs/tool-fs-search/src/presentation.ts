 ...
 * model-facing render consumes ({@link module:@nomix-ai/nomix-tool-fs-search/search-core}
 ...
 * final output budget (`nomix-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @nomix-ai/nomix-tool-fs-search/presentation
 */
 ...
 * ({@link module:@nomix-ai/nomix-tool-fs-search/grep} `formatGrepMatches`), so

...
/**
 * Keyless built-artifact smoke: plain Node imports `@nomix-ai/nomix-lsp` and
 * `@nomix-ai/nomix-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@nomix-ai/nomix-lsp')
      const LspLocal = await import('@nomix-ai/nomix-lsp-stdio')
      const { default: LocalFileSystem } = await import('@nomix-ai/nomix-fs-local')

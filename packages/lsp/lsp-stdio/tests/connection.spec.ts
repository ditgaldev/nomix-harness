...
import { LspConnection } from '@nomix-ai/nomix-lsp-stdio'
import type { ConnectionWriter } from '@nomix-ai/nomix-lsp-stdio/src/connection.ts'
import { scrubbedParentEnv } from '@nomix-ai/nomix-subprocess'
import { spawnSubprocess } from '@nomix-ai/nomix-subprocess-local/src/spawn.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit NOMIX_* env entries to the child', async () => {
    // A configured NOMIX_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'NOMIX_LSP_TEST_FACT', NOMIX_LSP_TEST_FACT: 'managed' })

/**
 * @nomix-ai/nomix-web-app — the browser-surface bundle's runtime glue plugin
 * plus the bundle patch (`cordis.patch.yml`, declared by the `nomix.bundle.patch`
...
import { addHarnessSourceSection } from '@nomix-ai/nomix-app-boot'
import * as FrontendStatic from '@nomix-ai/nomix-host-frontend-static'
import { launchEnvironmentOf } from '@nomix-ai/nomix-launch-environment'
import { scrubbedParentEnv } from '@nomix-ai/nomix-subprocess'
...
/** This nomix installation's root, from either this package's source or built entry. */
...
const NOMIX_WEB_URL = 'NOMIX_WEB_URL' as const
...
/** Model-visible orientation and acceptance boundary for sessions created through `nomix web`. */
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but client-plugin changes reload without a refresh only while '
    + '`pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify that watcher before promising automatic updates. '
    + 'Every other change — the apps/web shell and plain packages — requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
  return `You are interacting with the user through the Nomix Harness Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only nomix web injects window.__NOMIX_BOOT__. '
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [NOMIX_WEB_URL]: { description: 'Canonical local URL of the Nomix Harness Web GUI serving this session.' },
        },
        resolve: () => ({ [NOMIX_WEB_URL]: localWebUrl(runtimeCtx) }),
      })
    })
  }
  if (config.printUrl || handoffBrowser) {
    ...
    const announceReady = (): void => {
      ...
      if (config.printUrl) {
        console.log(`nomix web: ${webUrl}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
      }
      if (handoffBrowser) {
        console.log('nomix web: opening the default browser; pass --no-open to disable')

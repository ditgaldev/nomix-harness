...
function stageDist(): string {
  dist = mkdtempSync(join(tmpdir(), 'nomix-web-app-'))
...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    ctx.provide(NOMIX_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot([
    ...
    expect(log).toHaveBeenCalledWith('nomix web: opening the default browser; pass --no-open to disable')
    expect(openBrowser).toHaveBeenCalledWith('http://127.0.0.1:4567')
    expect(lifecycle).toEqual([
      'nomix web: http://127.0.0.1:4567 (LAN: http://192.168.1.5:4567)',
      'nomix web: opening the default browser; pass --no-open to disable',
      'open:http://127.0.0.1:4567',
    ])
    const assembly = await ctx.systemPrompt.assemble()
    expect(assembly.sections.find(entry => entry.name === 'harness:source')?.text).toContain('Nomix Harness implementation checkout')
    ...
    expect(webRuntime?.resolve()).toEqual({ NOMIX_WEB_URL: 'http://127.0.0.1:4567' })
  ...
  it('scrubs the helper environment and reports helper spawn or exit failures', async () => {
    ...
    vi.stubEnv('NOMIX_HOME', '/must-not-reach-browser')
    ...
    expect(options?.env).not.toHaveProperty('NOMIX_HOME')

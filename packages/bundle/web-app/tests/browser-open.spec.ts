...
import WebServer from '@nomix-ai/nomix-host-webserver'
...
afterEach(async () => {
  ...
  vi.unstubAllEnvs()
  Reflect.deleteProperty(globalThis, '__nomixWebAppApply')
  Reflect.deleteProperty(globalThis, '__nomixWebServer')
})

describe('web app browser startup', () => {
  it('opens the canonical URL only after the complete page is reachable', async () => {
    const root = mkdtempSync(join(tmpdir(), 'nomix-web-browser-open-'))
    tempRoots.push(root)
    ...
    writeFileSync(webserverModule, 'export default globalThis.__nomixWebServer\n')
    writeFileSync(webAppModule, [
      "export const name = 'fixture-web-app'",
      "export const inject = ['webServer']",
      'export const apply = (ctx, config) => globalThis.__nomixWebAppApply(ctx, config)',
      '',
    ].join('\n'))
    ...
    const globals = globalThis as unknown as {
      __nomixWebAppApply: typeof apply
      __nomixWebServer: typeof WebServer
    }
    globals.__nomixWebAppApply = apply
    globals.__nomixWebServer = WebServer

...
export class WebRuntime extends Service {
   ...
   * `$NOMIX_WEB_SEARCH_PROVIDER` / `$NOMIX_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.NOMIX_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.NOMIX_WEB_FETCH_PROVIDER
  }

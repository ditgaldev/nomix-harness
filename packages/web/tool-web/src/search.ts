...
import type { GenericCallView, JsonValue, ToolResult, WebSearchResultView, WebSource } from '@nomix-ai/nomix-tools'
import type { WebSearchResult, WebSearchSource } from '@nomix-ai/nomix-web'
import type {} from '@nomix-ai/nomix-system-prompt'

/**
 * Default upper bound on returned sources (the `searchMaxResults` config).
 * Owned by the consumer (not the provider or model), mirroring `nomix-tool-fs`'s

...
import { CallId, type StreamChunk } from '@nomix-ai/nomix-llm'
import SessionStore, { type SessionEvent } from '@nomix-ai/nomix-session'
import type { SessionPersistence } from '@nomix-ai/nomix-session-persistence'
import SessionPersistenceJsonl from '@nomix-ai/nomix-session-persistence-jsonl'
import SessionPersistenceSqlite from '@nomix-ai/nomix-session-persistence-sqlite'
...
describe('SQLite cross-backend differential behavior', () => {
  it('preserves ignorable logical events whose names match physical storage tags', async () => {
    ...
    const directory = await freshDirectory('nomix-sqlite-storage-tag-collision-')
  ...
  it('matches JSONL/Zstandard for every packed kind, scalar fallback, suffix, partition, and reopen', async () => {
    ...
    for (const [partitionIndex, sizes] of [[events.length], [1], [2, 1, 5, 3]].entries()) {
      const directory = await freshDirectory(`nomix-sqlite-matrix-${partitionIndex}-`)
  ...
  it('matches JSONL/Zstandard across randomized logical logs and append partitions', async () => {
    await fc.assert(fc.asyncProperty(randomWorkload, async ({ events, batchSizes }) => {
      const directory = await freshDirectory('nomix-sqlite-property-')
      for (const name of ['jsonl-zstd', 'sqlite'] as const) {

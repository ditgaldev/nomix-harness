...
import SubagentService, { seedDescriptorTurn, snapshotSubagentDescriptor } from '@nomix-ai/nomix-subagent'
import * as SubagentSpawn from '@nomix-ai/nomix-subagent-spawn-in-process'
...
for (const backend of backends) {
  describe(`${backend.name} Agent Teams recovery`, () => {
    it('reconciles a persisted child to active and a missing child to durable failed', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nomix-team-${backend.name.toLowerCase()}-`))
    ...
    it('reconciles a provisioning child whose initial prompt is durably pending', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nomix-team-pending-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('replays queued-minus-delivered mail in FIFO order without waking for quiet mail', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nomix-team-mail-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('acknowledges target-recorded mail after restart without delivering it twice', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nomix-team-dedup-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('acknowledges durably pending target mail without cold-resume duplication', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nomix-team-inbox-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)

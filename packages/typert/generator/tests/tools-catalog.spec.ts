...
import type { TypertContribution } from '@nomix-ai/nomix-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@nomix-ai/nomix-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const record = ctx.typert.getPackage('@nomix-ai/nomix-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@nomix-ai/nomix-tools', 'host')).toBeUndefined()
  })
})

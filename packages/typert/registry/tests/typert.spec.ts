...
} from '@nomix-ai/nomix-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@nomix-ai/nomix-typert-protocol'
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@nomix-ai/nomix-tools', 'ToolInput')).toBe('@nomix-ai/nomix-tools#ToolInput')
    expect(typertPackageKey('@nomix-ai/nomix-tools', 'host')).toBe('@nomix-ai/nomix-tools#host')
    expect(ctx.typert.get('@nomix-ai/nomix-tools#ToolInput')).toMatchObject({
      package: '@nomix-ai/nomix-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@nomix-ai/nomix-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@nomix-ai/nomix-tools', 'host')).toMatchObject({
      key: '@nomix-ai/nomix-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@nomix-ai/nomix-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@nomix-ai/nomix-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@nomix-ai/nomix-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@nomix-ai/nomix-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@nomix-ai/nomix-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@nomix-ai/nomix-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@nomix-ai/nomix-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@nomix-ai/nomix-tools#ToolInput')).not.toBe(projected)
  })

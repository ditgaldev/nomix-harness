/**
 * File-backed credentials provider over `$NOMIX_HOME/.credentials.yaml`, layered
 ...
 * > $NOMIX_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… nomix`, a CI
 ...
 * @module @nomix-ai/nomix-credentials-local
 */
...
import { withFileLock, writeFileAtomic } from '@nomix-ai/nomix-atomic-write'
import { canonicalizeWatchPath, resolveNomixHome } from '@nomix-ai/nomix-home-paths'
import { launchEnvironmentOf } from '@nomix-ai/nomix-launch-environment'
import { CredentialProvider, credentialRef, parseCredentialKey } from '@nomix-ai/nomix-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
  ...
  CredentialRef,
  ResolvedCredential,
} from '@nomix-ai/nomix-credentials'
import type { LaunchEnvironmentEntry } from '@nomix-ai/nomix-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveNomixHome(config.nomixHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `nomix-atomic-write`, this is a
...
/** File-backed credentials provider (`$NOMIX_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start nomix from instead',
      )
    }
  }

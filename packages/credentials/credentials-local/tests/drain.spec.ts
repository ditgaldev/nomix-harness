...
import { credentialKey, credentialRef } from '@nomix-ai/nomix-credentials'
...
vi.mock('@nomix-ai/nomix-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@nomix-ai/nomix-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@nomix-ai/nomix-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
  mocked.__setGate(next)
}

const KEY = credentialRef('NOMIX_CRED_DRAIN_A')
const OTHER = credentialRef('NOMIX_CRED_DRAIN_B')
...
describe('write-drain teardown', () => {
  it('lets the in-flight write land and fails the queued one after disposal', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nomix-credentials-drain-'))
  ...
  it('fails a queued record write after disposal on the same terms', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nomix-credentials-drain-record-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const ctx = new Context()
    const fiber = ctx.plugin(LocalCredentialProvider, { path: join(dir, '.credentials.yaml'), watch: false })
    await fiber

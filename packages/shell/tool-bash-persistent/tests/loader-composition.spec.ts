...
suite('persistent Bash through a real cordis.yml Loader composition', () => {
  it('preserves cwd and environment across calls', async () => {
    root = await mkdtemp(join(tmpdir(), 'nomix-persistent-bash-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@nomix-ai/nomix-tool-bash-persistent', ToolBashPersistent],
    ])
    ...
    expect(observed).not.toContain('NOMIX_PERSISTENT_BASH')
    ...
    expect(multiline).not.toContain('NOMIX_PERSISTENT_BASH')

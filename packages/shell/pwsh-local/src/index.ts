 ...
 *
 * @module @nomix-ai/nomix-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors nomix-bash-local call-for-call by
...
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@nomix-ai/nomix-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@nomix-ai/nomix-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@nomix-ai/nomix-subprocess'
import { installSettingsSection } from '@nomix-ai/nomix-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@nomix-ai/nomix-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of nomix-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `nomix-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.nomixEnv },
    }
  }
   ...
   * `nomix-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@nomix-ai/nomix-pwsh-sandbox`.

...
工具无关的 shell 环境插件：拥有 `ctx.shellEnv` 注册表，管理受信任的、每次执行收集的 `NOMIX_*` 变量，供模型可见的 shell 工具（`nomix-tool-bash`、`nomix-tool-pwsh`）收集进每次 shell 调用的环境。内置 shell 事实（`NOMIX_HOME`、`NOMIX_SHELL=1`、`NOMIX_SESSION_ID`）归注册表自身所有；其他插件可以注册额外的可枚举事实，注册随插件纤维（fiber）释放，重复所有权或未声明的运行时键会响亮失败。
...
覆盖层根据当前 `ToolExecution` 计算，并通过专用的 `ShellExecRequest.nomixEnv` 通道传递。本地执行器在合并该快照前移除所有继承的 `NOMIX_*`，因此嵌套 harness 与并发的父子 agent 无法泄漏陈旧身份。`process.env` 永不被修改。shell 工具的描述只教授通用的 `$NOMIX_*` 约定，而不是点名持久化相关的变量或添加常驻的 system-prompt 段落。

## Model Experience

通过 shell 工具（`nomix-tool-bash`、`nomix-tool-pwsh`）间接产生影响；这些工具会把该注册表的受管 `NOMIX_*` 快照收集进每次 shell 工具调用。
...
- **`list()` 只枚举 contributor 声明的变量** — 注册表自有的内置键（`NOMIX_HOME`、`NOMIX_SHELL`、`NOMIX_SESSION_ID`）不包含在内，因此诊断、prompt 或 UI 代码不得把 `list()` 当作完整的环境目录。

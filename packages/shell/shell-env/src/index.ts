 ...
 * trusted, per-execution `NOMIX_*` variables consumed by the model-facing shell
 * tools (`nomix-tool-bash`, `nomix-tool-pwsh`). Built-in shell facts are owned by
 * the registry itself while plugins can register additional, enumerable facts
 * with effect-scoped disposal.
 *
 * @module @nomix-ai/nomix-shell-env
 */

import { Service, type Context } from '@nomix-ai/cordis'
import z from '@nomix-ai/schemastery'
import { NOMIX_ENV_PREFIX } from '@nomix-ai/nomix-shell'
import type { NomixEnvironment, NomixEnvironmentKey } from '@nomix-ai/nomix-shell'
import { NOMIX_HOME_ENV, resolveNomixHome } from '@nomix-ai/nomix-home-paths'
...
/** Model-visible metadata for one managed `NOMIX_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `NOMIX_*` keys this contributor may return. */
  variables: Readonly<Record<NomixEnvironmentKey, BashEnvVariable>>
  /**
   ...
   */
  resolve(execution: ToolExecution): Readonly<Partial<Record<NomixEnvironmentKey, string>>>
}
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `NOMIX_*` environment variable name. */
  key: NomixEnvironmentKey
}

const NOMIX_SHELL_KEY = `${NOMIX_ENV_PREFIX}SHELL` as const
const NOMIX_SESSION_ID_KEY = `${NOMIX_ENV_PREFIX}SESSION_ID` as const
const NOMIX_SESSION_JSONL_KEY = `${NOMIX_ENV_PREFIX}SESSION_JSONL` as const
const RESERVED_BASH_ENV_KEYS = new Set<NomixEnvironmentKey>([
  NOMIX_HOME_ENV,
  NOMIX_SHELL_KEY,
  NOMIX_SESSION_ID_KEY,
])
const BASH_ENV_KEY_SUFFIX = /^[A-Z][A-Z0-9_]*$/

/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `NOMIX_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `NOMIX_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly keyOwners = new Map<NomixEnvironmentKey, string>()
  private readonly nomixHome: string
  ...
  constructor(ctx: Context, config: Config = {}) {
    super(ctx, 'shellEnv')
    this.nomixHome = resolveNomixHome(config.nomixHome)
  }
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      const variables = Object.entries(contributor.variables) as [NomixEnvironmentKey, BashEnvVariable][]
      for (const [key, variable] of variables) {
        if (!key.startsWith(NOMIX_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(NOMIX_ENV_PREFIX.length))) {
  ...
  collect(execution: ToolExecution): NomixEnvironment {
    const values: Record<NomixEnvironmentKey, string> = {
      [NOMIX_HOME_ENV]: this.nomixHome,
      [NOMIX_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[NOMIX_SESSION_ID_KEY] = execution.agent.session.header.id
    }
    ...
    for (const contributor of [...this.contributors.values()].sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      for (const [rawKey, value] of Object.entries(resolved)) {
        const key = rawKey as NomixEnvironmentKey
  ...
  list(): BashEnvVariableInfo[] {
    return [...this.contributors.values()]
      .flatMap(contributor => Object.entries(contributor.variables).map(([key, variable]) => ({
        ...
        key: key as NomixEnvironmentKey,
      })))
 ...
 * shell-agnostic persistence contributor (`NOMIX_SESSION_JSONL`).
 * @param ctx - Cordis context that owns the service and registrations.
 * @param config - home-directory configuration for the built-in variables.
 */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  registry.register({
    ...
    resolve(execution) {
      ...
      return location?.kind === 'jsonl' ? { [NOMIX_SESSION_JSONL_KEY]: location.path } : {}
    },
  })
}

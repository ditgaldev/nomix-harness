...
import type { ShellRunResult, CollectedOutput } from '@nomix-ai/nomix-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@nomix-ai/nomix-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@nomix-ai/nomix-sandbox'
...
import type { Config } from '@nomix-ai/nomix-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'nomix-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'NOMIX_WRAP=1', 'bash', '-c', 'printf "%s" "$NOMIX_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$NOMIX_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'nomix-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.NOMIX_ORDER_FILE, "runner\\n");',
    ...
    try {
      const result = await bash.run(bash.resolve({
        command: 'true',
        env: { BASH_ENV: hook },
        nomixEnv: { NOMIX_ORDER_FILE: order },
      }))
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'nomix-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'nomix-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)

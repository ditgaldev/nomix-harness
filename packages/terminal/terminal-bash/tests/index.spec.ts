...
import AgentRegistry, { Inbox, type Agent } from '@nomix-ai/nomix-agent'
...
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@nomix-ai/nomix-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@nomix-ai/nomix-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@nomix-ai/nomix-terminal-bash'
import { ENCODING_PREAMBLE } from '@nomix-ai/nomix-pwsh-local'
import * as ptyLocal from '@nomix-ai/nomix-terminal-bash'
import type { ResolvedConfig } from '@nomix-ai/nomix-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@nomix-ai/nomix-terminal-bash/src/session.ts'
import { SubprocessRuntime } from '@nomix-ai/nomix-subprocess'
import type {
  SubprocessHandle,
  SubprocessSpawnSpec,
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo nomix> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo nomix> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', NOMIX_SHELL: '1', NOMIX_SESSION_ID: 'agent', NOMIX_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'nomix> ' : 'PowerShell 7.6.4\n',
  ...
  it('forwards the spawn signal into the pwsh bootstrap sends', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'nomix> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
    ...
    expect(spawned.motd).toBe('nomix> ')

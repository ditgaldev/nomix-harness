...
import { normalizeTerminalText, TerminalSanitizer } from '@nomix-ai/nomix-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07nomix> ')).toEqual({ text: 'nomix> ', prompt: true, promptTail: 'nomix> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('nomix> ')).toEqual({ text: 'nomix> ', prompt: false, promptTail: 'nomix> ' })
  })

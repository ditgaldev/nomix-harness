...
import { TerminalBackendCleanupError } from '@nomix-ai/nomix-terminal'
...
import { effectiveSandboxMode } from '@nomix-ai/nomix-sandbox-policy'
...
function childEnvironment(spec: TerminalBackendSpawnSpec, dialect: ShellDialect): Record<string, string> {
  ...
  const common = {
    ...
    GIT_PAGER: 'cat',
    NOMIX_SHELL: '1',
    NOMIX_SESSION_ID: spec.owner.id,
    NOMIX_PTY_SESSION_ID: spec.sessionId,
  }
  if (dialect === 'pwsh') {

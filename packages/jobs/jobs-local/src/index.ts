 ...
 * @module @nomix-ai/nomix-jobs-local
 */

import { Context } from '@nomix-ai/cordis'
import z from '@nomix-ai/schemastery'
import type { Agent } from '@nomix-ai/nomix-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@nomix-ai/nomix-scope'
import type { ScopeLayer } from '@nomix-ai/nomix-scope'
import { deadline, timeoutOf } from '@nomix-ai/nomix-timeout'
import { JobRegistry, JobId } from '@nomix-ai/nomix-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@nomix-ai/nomix-jobs'
 ...
 * `@nomix-ai/nomix-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @nomix-ai/nomix-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @nomix-ai/nomix-agent)')
    }

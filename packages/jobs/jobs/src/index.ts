 ...
 * process-local registry lives in `@nomix-ai/nomix-jobs-local`.
 * @module @nomix-ai/nomix-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@nomix-ai/nomix-jobs is the abstract job registry seam; load an implementation such as @nomix-ai/nomix-jobs-local instead')
    }
    super(ctx, 'jobs')
  }

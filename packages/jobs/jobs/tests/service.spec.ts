...
import { JobId, JobRegistry } from '@nomix-ai/nomix-jobs'
import type {
 ...
 * behavior suite lives with `@nomix-ai/nomix-jobs-local`.
 */
...
describe('JobRegistry seam', () => {
  ...
  it('mounting the abstract seam directly fails loudly at load (stale-composition fence)', async () => {
    ...
    await expect(ctx.plugin(JobRegistry as unknown as typeof StubJobRegistry))
      .rejects.toThrow(/abstract job registry seam; load an implementation such as @nomix-ai\/nomix-jobs-local/)
  })
})

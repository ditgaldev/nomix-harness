 ...
 *
 * @module nomix-llm-pi-ai/replay
 */

import { LlmError } from '@nomix-ai/nomix-llm'
import type { Message, ModelMessageSource, ReplayEnvelope } from '@nomix-ai/nomix-llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'nomix-foreign',
    provider: source?.provider ?? 'nomix-foreign',
    model: source?.model ?? 'nomix-foreign',
    usage: emptyPiUsage(),

...
import type { CredentialKey, CredentialProvider, CredentialRecord } from '@nomix-ai/nomix-credentials'
import { launchEnvironmentOf } from '@nomix-ai/nomix-launch-environment'
import { LlmError } from '@nomix-ai/nomix-llm'
...
function writableStore(ctx: Context): CredentialProvider {
  ...
  if (credentials === undefined) {
    throw new LlmError(
      ...
      + ' credential a sign-in produces; mount one (nomix-credentials-local) to sign in',
      'NO_CREDENTIAL_STORE',
    )
  }
  return credentials

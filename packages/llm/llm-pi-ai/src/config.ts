...
import { resolveRetryPolicy, RetryPolicySchema } from '@nomix-ai/nomix-llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@nomix-ai/nomix-llm'
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with nomix-llm-retry',
    )
  }
}

...
import LlmRuntime, { createUserMessage, ReasoningEffortId } from '@nomix-ai/nomix-llm'
...
import { PiAiAdapter } from '@nomix-ai/nomix-llm-pi-ai'
...
/** A throwaway $NOMIX_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nomix-pi-catalog-'))
  homes.push(dir)

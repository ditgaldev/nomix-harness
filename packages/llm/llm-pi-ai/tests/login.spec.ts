...
import type { AuthorizationInteraction, AuthorizationNotice, AuthorizationPrompt } from '@nomix-ai/nomix-authorization'
import LocalCredentialProvider from '@nomix-ai/nomix-credentials-local'
import type { CredentialKey } from '@nomix-ai/nomix-credentials'
...
async function harness(): Promise<Context> {
  const dir = await mkdtemp(join(tmpdir(), 'nomix-pi-login-'))
  dirs.push(dir)

...
import LlmRuntime, { createMessage, createUserMessage } from '@nomix-ai/nomix-llm'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  root = await mkdtemp(join(tmpdir(), 'nomix-pi-composition-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@nomix-ai/nomix-llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',

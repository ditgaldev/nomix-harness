...
import { AttachmentId, ImageVariantId } from '@nomix-ai/nomix-attachment'
import type { AttachmentStore, ImageAttachmentRef, RequestImageAttachment } from '@nomix-ai/nomix-attachment'
import { createLaunchEnvironmentSnapshot } from '@nomix-ai/nomix-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@nomix-ai/nomix-anonymous-user-id'
import { SessionId } from '@nomix-ai/nomix-session'
import * as LlmDeepSeek from '@nomix-ai/nomix-llm-deepseek'
import { DeepSeekAdapter, resolveAdapterOptions } from '@nomix-ai/nomix-llm-deepseek'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'nomix-llm-deepseek-'))
  vi.stubEnv('NOMIX_HOME', testHome)
})
...
describe('DeepSeekAdapter against a mock server', () => {
  it('streams a text generation end to end through the assembler', async () => {
    ...
    expect(server.headers[0]?.['x-nomix-harness-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-nomix-harness-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-nomix-harness-compact')
  })
  ...
  it('uploads a durable image once and sends only its Files API id to the vision model', async () => {
    ...
    expect(server.fileRequests).toEqual([{
      method: 'POST',
      path: '/files',
      filename: `nomix-${'a'.repeat(16)}-${'b'.repeat(8)}.png`,
      bytes: 3,
    }])
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    expect(server.headers[1]?.['x-nomix-harness-compact']).toBe('1')
  })
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-nomix-harness-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-nomix-harness-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-nomix-harness-compact']).toBe('1')
  })
...
describe('plugin registration and config', () => {
  ...
  it('takes DEEPSEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.nomix/.env', values: { DEEPSEEK_BASE_URL: 'https://user.example' } },
    ])

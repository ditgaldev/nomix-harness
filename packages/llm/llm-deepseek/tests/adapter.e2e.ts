...
import LlmRuntime, { createUserMessage, CallId, ReasoningEffortId, createMessage } from '@nomix-ai/nomix-llm'
import type { Message, ToolSchema } from '@nomix-ai/nomix-llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@nomix-ai/nomix-attachment'
import type {
...
import type { Config } from '@nomix-ai/nomix-llm-deepseek'
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'nomix-e2e-user-id-'))
  vi.stubEnv('NOMIX_HOME', identityHome)
})
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('llm-deepseek e2e (real API)', () => {
  ...
  it('serves a real request with the key held only by a credentials-local document', async () => {
    const key = process.env.DEEPSEEK_API_KEY
    if (key === undefined) throw new Error('e2e ran without DEEPSEEK_API_KEY')
    const dir = await mkdtemp(join(tmpdir(), 'nomix-e2e-credentials-'))
    try {

...
import LocalCredentialProvider from '@nomix-ai/nomix-credentials-local'
import { settingsNamespace } from '@nomix-ai/nomix-settings'
import FileSettingsProvider from '@nomix-ai/nomix-settings-file'
...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'nomix-llm-composition-'))
  vi.stubEnv('NOMIX_HOME', root)
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@nomix-ai/nomix-settings-file', FileSettingsProvider],
    ['@nomix-ai/nomix-credentials-local', LocalCredentialProvider],
    ['@nomix-ai/nomix-llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-deepseek real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-nomix-harness-user-id']).toBe(getOrCreateAnonymousUserId())

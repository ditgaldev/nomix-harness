...
import { TypertLookupFailure } from '@nomix-ai/nomix-typert-protocol'
import TypertRegistry from '@nomix-ai/nomix-typert-registry'
import { createUserMessage, MessageId } from '@nomix-ai/nomix-llm'
import type { Agent } from '@nomix-ai/nomix-agent'
import UserQuestionService from '@nomix-ai/nomix-user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@nomix-ai/nomix-session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@nomix-ai/nomix-session-persistence'
import type { RpcRequest } from '@nomix-ai/nomix-host-apiproxy/api/rpc'
...
describe('sessions.list cold merge', () => {
  it('verifies only small possibly-blank artifacts and treats every unavailable probe as visible', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'nomix-cold-'))
  ...
  it('replaces a probed cold row with the live Session that attached during the read', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'nomix-cold-race-'))
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@nomix-ai/nomix-session').Session

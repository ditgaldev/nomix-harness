...
import LlmRuntime, { LlmAdapter } from '@nomix-ai/nomix-llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@nomix-ai/nomix-llm'
import { SettingsProvider, settingsNamespace } from '@nomix-ai/nomix-settings'
import type { SettingsNamespace } from '@nomix-ai/nomix-settings'
import { CredentialProvider } from '@nomix-ai/nomix-credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@nomix-ai/nomix-credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@nomix-ai/nomix-agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('nomix-settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('nomix-credentials-local')
  })

...
import { installModelSelection } from '@nomix-ai/nomix-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@nomix-ai/nomix-agent'
import type {} from '@nomix-ai/nomix-agent-presets/types'
import { AttachmentError, admitEncodedImages } from '@nomix-ai/nomix-attachment'
import type { ImageAttachmentRef } from '@nomix-ai/nomix-attachment'
import { createUserMessage, freezeMessage, ReasoningEffortId } from '@nomix-ai/nomix-llm'
import { errorChain } from '@nomix-ai/nomix-llm'
import type { ContentBlock, MessageSource } from '@nomix-ai/nomix-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@nomix-ai/nomix-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@nomix-ai/nomix-session'
import type { SessionPersistence } from '@nomix-ai/nomix-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@nomix-ai/nomix-session-query'
import { SubagentError } from '@nomix-ai/nomix-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@nomix-ai/nomix-subagent'
import { isUserInvocable } from '@nomix-ai/nomix-skill'
import type { Workspace, WorkspaceRecord } from '@nomix-ai/nomix-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@nomix-ai/nomix-workspace'
// Type-only: brings the `ctx.tools` Context merge into this program (viewFor reads presenters).
import {
  InvalidPresetIdError, PresetExistsError, PresetMountError,
  PresetNotWritableError, resolveSessionPreset, UnknownPresetError,
} from '@nomix-ai/nomix-agent-presets'
import type { PresetBearingSession } from '@nomix-ai/nomix-agent-presets'
...
import type { SessionRawArtifact } from '@nomix-ai/nomix-session-persistence'
import {
...
import type {} from '@nomix-ai/nomix-session-projection'
...
import type {} from '@nomix-ai/nomix-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@nomix-ai/nomix-goal'
import type { GoalRef as CoreGoalRef } from '@nomix-ai/nomix-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@nomix-ai/nomix-commands'
...
import type {} from '@nomix-ai/nomix-cordis-host-runner/types'
import type {} from '@nomix-ai/nomix-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@nomix-ai/nomix-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@nomix-ai/nomix-settings'
import { credentialRef } from '@nomix-ai/nomix-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@nomix-ai/nomix-session-title'
import type { CallId } from '@nomix-ai/nomix-llm/brand'
import type { ScopeKey } from '@nomix-ai/nomix-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@nomix-ai/nomix-user-approval'
...
} from '@nomix-ai/nomix-user-questions'
import { UserQuestionError } from '@nomix-ai/nomix-user-questions'
import { DirectoryPickerError } from '@nomix-ai/nomix-host-directory-picker'
import {
...
} from '@nomix-ai/nomix-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @nomix-ai/nomix-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: nomix-llm depends on nomix-attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @nomix-ai/nomix-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @nomix-ai/nomix-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @nomix-ai/nomix-credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @nomix-ai/nomix-session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in nomix-client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts nomix-skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @nomix-ai/nomix-skill', details: {} })
        }

/**
 * Package-owned invariant companion for `@nomix-ai/nomix-host-webserver`.
 * @module @nomix-ai/nomix-host-webserver/invariant
 */
...
const PACKAGE_NAME = '@nomix-ai/nomix-host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__nomix_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__nomix_invariant_upgrade_probe__', handler: () => {} }

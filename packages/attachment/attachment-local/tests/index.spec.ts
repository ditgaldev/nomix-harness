...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const nomixHome = await mkdtemp(join(tmpdir(), 'nomix-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nomixHome })
      ...
      await rm(nomixHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const nomixHome = await mkdtemp(join(tmpdir(), 'nomix-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nomixHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const nomixHome = await mkdtemp(join(tmpdir(), 'nomix-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nomixHome })
  ...
  it('prepares every batch member before any write', async () => {
    const nomixHome = await mkdtemp(join(tmpdir(), 'nomix-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nomixHome, normalizedImageMaxBytes: 1 })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const nomixHome = await mkdtemp(join(tmpdir(), 'nomix-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nomixHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { nomixHome, maxImageBytes: 1 })

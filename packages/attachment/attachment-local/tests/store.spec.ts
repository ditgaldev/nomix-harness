...
import type { ImageAttachmentLimits } from '@nomix-ai/nomix-attachment'
...
async function root(): Promise<string> {
  const value = await mkdtemp(join(tmpdir(), 'nomix-attachment-'))
  roots.push(value)
...
describe('local attachment store', () => {
  it.skipIf(process.platform === 'win32')('syncs every object ancestor up to the durable boundary before returning', async () => {
    ...
    // Each process first proves NOMIX_HOME durable all the way to the filesystem

...
describe('request image verification', () => {
  it('rejects an encoded request whose decoded facts disagree with the encoder result', async () => {
    const nomixHome = await mkdtemp(join(tmpdir(), 'nomix-request-verification-'))
    homes.push(nomixHome)
    const attachments = new LocalAttachmentStore(new Context(), { nomixHome })

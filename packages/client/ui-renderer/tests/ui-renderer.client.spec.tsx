...
import { TestSessions, TestWorkspaces } from '@nomix-ai/nomix-client-test-runtime'
import type { Stabilizer } from '@nomix-ai/nomix-client-test-runtime'
import { apply as nodeApply } from '@nomix-ai/nomix-client-ui-renderer'
...
describe('UI renderer plugin', () => {
  ...
  it('hydrates the boot page before switching to the assembled application', async () => {
    ...
    slots.register({ name: 'root' }, () => <div data-testid="root-probe" />)
    const el = container()
    el.innerHTML = '<div class="boot" data-nomix-boot=""><div><div class="spinner" data-nomix-boot-spinner="" style="--nomix-boot-arc: 180deg"></div><div>Loading plugins…</div></div></div>'

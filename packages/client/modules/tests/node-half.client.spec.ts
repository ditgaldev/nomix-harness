...
import { renderIndexInjections, type WebServer, type WebRoute } from '@nomix-ai/nomix-host-webserver'
...
const MODULES_ID = '@nomix-ai/nomix-client-modules'
const RUNTIME_ID = '@nomix-ai/nomix-client-runtime'
...
function writePackage(
  packageName: string,
  metadata: Record<string, unknown> = { nomix: { client: { platform: 'web' } } },
): string {
  root ??= realpathSync(mkdtempSync(join(tmpdir(), 'nomix-client-modules-')))
...
function writeBuiltPackage(packageName: string, client: Record<string, unknown>): void {
  const clientPath = writePackage(packageName, { nomix: { client: { platform: 'web', ...client } } })
...
describe('HTML bootstrap facade', () => {
  it('precedes blocking preloads and the boot graph, then becomes the live registration target', async () => {
    ...
    const graphAt = html.indexOf('globalThis["__NOMIX_BOOT__"] = ')
...
describe('shared module declarations', () => {
  ...
  it('rejects a non-array external', () => {
    ...
    expect(() => construct([packageName]))
      .toThrow(`client-modules: ${packageName} nomix.client.external must be a string array`)
  })
})
...
describe('module graph order', () => {
  ...
  it('leaves a request no row answers to the static assembly channel', () => {
    expect(ids(orderByModuleGraph([
      entry('consumer', { external: ['@nomix-ai/cordis'] }),
      entry('other'),

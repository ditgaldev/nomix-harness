...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@nomix-ai/nomix-llm/message'
import { CallId } from '@nomix-ai/nomix-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@nomix-ai/nomix-attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@nomix-ai/nomix-session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@nomix-ai/nomix-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@nomix-ai/nomix-commands/types'
import { deriveEventMessage, foldSurface } from '@nomix-ai/nomix-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@nomix-ai/nomix-host-apiproxy/api'
...
function buildAlphaLog(): SessionEvent[] {
  ...
  toolTurn(70, 'web_search', '{"queries":["nomix harness architecture"]}', 'Search results for nomix harness architecture.')
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ...
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@nomix-ai/nomix-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@nomix-ai/nomix-tool-read'\n" }],
  ])
      ...
      const value = sessions
        ...
        .map((item) => {
          ...
          return {
            ...
            mention: `@[${label}](nomix-session:${encoded})`,
          }
        })

 ...
 * determinism
 * lives host-side — the pre-step boundary (`nomix-tool-skill`) recognizes a
...
import type { ConnectionHandle, SessionId, SkillEntry } from '@nomix-ai/nomix-api-remotes/client'
import type { ClientContext, ISessions } from '@nomix-ai/nomix-client-runtime/client'
import type { InputTriggerServiceContract, InputTriggerSource } from '@nomix-ai/nomix-client-ui-input-trigger/client'
// Type-only: pulls the locale plugin's Context merge (ctx.locale).
import type {} from '@nomix-ai/nomix-client-locale/client'
...
export function apply(ctx: ClientContext): void {
  ...
  const source: InputTriggerSource = {
    ...
    onPick({ candidate }) {
      ...
      // pre-step boundary (nomix-tool-skill) recognizes the leading /name and

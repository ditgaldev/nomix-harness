 ...
 * @module @nomix-ai/nomix-client-web/src/boot
 */
import { Context } from '@nomix-ai/cordis'
import Loader from '@nomix-ai/cordis-plugin-loader'
import type {
  BootManifest, ClientModuleCreateOptions, ClientModuleSystem, NomixWindow,
} from '@nomix-ai/nomix-client-modules/client'
import type {} from '@nomix-ai/nomix-client-ui-renderer/client'
...
export class AppWebEntry {
  ...
  async run(): Promise<void> {
    try {
      ...
      // is `ClientTransportHooks`, owned by @nomix-ai/nomix-client-connection;
      // this structural slice reads one optional member without adding a
      // package edge.
      const transport = (globalThis as {
        __NOMIX_TRANSPORT__?: { loadBundle?: ClientModuleCreateOptions['loadBundle'] }
      }).__NOMIX_TRANSPORT__
      this.modules = moduleLoader.create({
        boot: win.__NOMIX_BOOT__,
  ...
  private async prefetchImmediateTier(): Promise<void> {
    ...
    const transport = (globalThis as {
      __NOMIX_TRANSPORT__?: { loadBundle?: unknown }
    }).__NOMIX_TRANSPORT__

...
afterEach(cleanup)
import { AppRoot } from '@nomix-ai/nomix-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@nomix-ai/nomix-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@nomix-ai/nomix-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@nomix-ai/nomix-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })

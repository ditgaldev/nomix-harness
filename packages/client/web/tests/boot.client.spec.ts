...
import type {
  ClientBundleRegistration, ClientModuleCreateOptions, ClientModuleLoaderTarget, NomixWindow,
  WebBootEntry,
...
describe('bootstrap failure rendering', () => {
  ...
  it('renders a malformed boot manifest', async () => {
    await expectBootFailure(() => {
      installFacade()
      delete win.__NOMIX_BOOT__
    }, 'window.__NOMIX_BOOT__ is missing or not an object')
  })
  ...
  it('renders a module-system construction failure', async () => {
    await expectBootFailure(() => {
      ...
      win.__NOMIX_BOOT__ = { rev: 'graph', entries: [duplicate, duplicate] }
...
describe('plugin activation', () => {
  it('allows a modules-dependent row to be created before the modules row', async () => {
    ...
    win.__NOMIX_BOOT__ = { rev: 'graph', entries }

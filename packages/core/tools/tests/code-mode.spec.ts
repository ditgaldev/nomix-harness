...
import { createUserMessage, CallId  } from '@nomix-ai/nomix-llm'
import { createScope } from '@nomix-ai/nomix-scope'
import type { Scope } from '@nomix-ai/nomix-scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@nomix-ai/nomix-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@nomix-ai/nomix-tools'
import type { Agent } from '@nomix-ai/nomix-agent'
import { Session, SessionId } from '@nomix-ai/nomix-session'
import type { JsonValue, SessionEventMap } from '@nomix-ai/nomix-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@nomix-ai/nomix-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)

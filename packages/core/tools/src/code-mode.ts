 ...
 * @module @nomix-ai/nomix-tools/src/code-mode
 */

import { CallId, createUserMessage, HarnessError } from '@nomix-ai/nomix-llm'
import type { ContentBlock } from '@nomix-ai/nomix-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@nomix-ai/nomix-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`nomix-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}

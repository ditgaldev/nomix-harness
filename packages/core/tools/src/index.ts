 ...
 * execution pipeline.
 * @module @nomix-ai/nomix-tools
 */

import { Context, Service } from '@nomix-ai/cordis'
import z from '@nomix-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@nomix-ai/nomix-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@nomix-ai/nomix-scope'
import type { CallId, ContentBlock, ToolSchema } from '@nomix-ai/nomix-llm'
import { assertNever, deepFreeze, HarnessError } from '@nomix-ai/nomix-llm'
import type { Agent } from '@nomix-ai/nomix-agent'
import { snapshotJsonValue } from '@nomix-ai/nomix-session'
import type { JsonValue, UserMessage } from '@nomix-ai/nomix-session'
import type { ToolProviderResult } from '@nomix-ai/nomix-system-prompt'
import type { CodeRuntime } from '@nomix-ai/nomix-code-runtime'
 ...
 * instead of deriving them: the seam's `nomix-code-runtime` README pair, its
...
export type { JsonValue } from '@nomix-ai/nomix-session'
...
// lives in its own UI-facing module; re-export it so `@nomix-ai/nomix-tools`
...
declare module '@nomix-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@nomix-ai/nomix-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@nomix-ai/nomix-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@nomix-ai/nomix-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@nomix-ai/nomix-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@nomix-ai/nomix-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `nomix-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`nomix-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`nomix-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @nomix-ai/nomix-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`nomix-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `nomix-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('nomix.tool.execution') as ToolExecutionToken
}

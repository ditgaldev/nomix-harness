 ...
 *
 * @module @nomix-ai/nomix-agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@nomix-ai/cordis'
import type { Fiber } from '@nomix-ai/cordis'
import { AsyncLocalStorage } from 'node:async_hooks'
import { isPromise } from 'node:util/types'
import { scopeTarget } from '@nomix-ai/nomix-scope'
import type { Scoped } from '@nomix-ai/nomix-scope'
import type { SessionEvent, SessionId } from '@nomix-ai/nomix-session'
...
declare module '@nomix-ai/cordis' {
  interface Context {
     ...
     * `nomix-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in nomix-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `nomix-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `nomix-agent-loop` package.
 */
 ...
 * (`@nomix-ai/nomix-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@nomix-ai/nomix-agent#Agent',

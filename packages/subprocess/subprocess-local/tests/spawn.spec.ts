...
import type { SubprocessHandle, SubprocessOutputReader } from '@nomix-ai/nomix-subprocess'
...
function shellArgv(command: string): string[] {
  ...
  switch (command) {
    ...
    case 'echo "[${NOMIX_STALE:-absent}|$NOMIX_SHELL|$NOMIX_SESSION_ID]"':
      return node('console.log("[" + [process.env.NOMIX_STALE ?? "absent", process.env.NOMIX_SHELL, process.env.NOMIX_SESSION_ID].join("|") + "]")')
    case 'echo "[${NOMIX_TEST_API_KEY:-absent}|${NOMIX_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${NOMIX_TEST_PLAIN:-absent}]"':
      return node('console.log("[" + [process.env.NOMIX_TEST_API_KEY ?? "absent", process.env.NOMIX_TEST_TOKEN ?? "absent", process.env.SUBPROCESS_TEST_PASSWORD ?? "absent", process.env.NOMIX_TEST_PLAIN ?? "absent"].join("|") + "]")')
...
const spillDir = mkdtempSync(join(tmpdir(), 'nomix-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-nomix-test' })).done)
...
describe.skipIf(process.platform === 'win32')('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@nomix-ai/cordis')
    const { default: LocalSubprocessRuntime } = await import('@nomix-ai/nomix-subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('childEnv keeps the POSIX spread on non-Windows hosts', () => {
    ...
    try {
      expect(childEnv({ NOMIX_X: '1' }).NOMIX_X).toBe('1')
    } finally {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-nomix-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-nomix-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient NOMIX env vars from child processes', async () => {
    process.env.NOMIX_TEST_API_KEY = 'super-secret'
    process.env.NOMIX_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.NOMIX_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${NOMIX_TEST_API_KEY:-absent}|${NOMIX_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${NOMIX_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.NOMIX_TEST_API_KEY
      delete process.env.NOMIX_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.NOMIX_TEST_PLAIN
    }
  })

  it('forwards explicit NOMIX_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient NOMIX_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.NOMIX_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${NOMIX_STALE:-absent}|$NOMIX_SHELL|$NOMIX_SESSION_ID]"', {
        env: { NOMIX_SHELL: '1', NOMIX_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.NOMIX_STALE
    }
  })
  ...
  it.skipIf(process.platform === 'win32')('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/nomix-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it.skipIf(process.platform === 'win32')('defaults spills into a private per-process directory', async () => {
    ...
    expect(dir).toMatch(/nomix-subprocess-/)

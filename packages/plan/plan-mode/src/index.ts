 ...
 *
 * @module @nomix-ai/nomix-plan-mode
 */
...
import type { Session, SessionEvent, UserMessage } from '@nomix-ai/nomix-session'
...
import type { CommandId } from '@nomix-ai/nomix-commands'
...
declare module '@nomix-ai/nomix-session/types' {
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('nomix-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'nomix-plan-mode: close service lifetime')

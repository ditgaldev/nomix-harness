...
import * as CommandFeedback from '@nomix-ai/nomix-command-feedback'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    root = await mkdtemp(join(tmpdir(), 'nomix-command-feedback-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@nomix-ai/nomix-commands', CommandRuntime],
      ['@nomix-ai/nomix-command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { NOMIX_HOME: root } })

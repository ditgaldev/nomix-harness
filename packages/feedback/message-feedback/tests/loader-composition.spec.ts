...
import * as StorageDomain from '@nomix-ai/nomix-storage-domain'
import * as StorageJson from '@nomix-ai/nomix-storage-json'
import { remoteMethods } from '@nomix-ai/nomix-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@nomix-ai/nomix-session-persistence-jsonl', JsonlSessionPersistence],
    ['@nomix-ai/nomix-storage', Storage],
    ['@nomix-ai/nomix-storage-json', StorageJson],
    ['@nomix-ai/nomix-storage-domain', StorageDomain],
    ['@nomix-ai/nomix-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'nomix-message-feedback-loader-'))

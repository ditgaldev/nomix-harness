...
import type { CommandHandle, CommandResult, Sandbox } from '@nomix-ai/nomix-e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
  ...
  SubprocessSpawnSpec,
} from '@nomix-ai/nomix-subprocess'
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$nomix_e2b_env_bin" -i "$nomix_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$nomix_e2b_tee" --output-error=warn-nopipe >("$nomix_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$nomix_e2b_tee" --output-error=warn-nopipe >("$nomix_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'nomix_e2b_rm=$7',
    'shift 7',
    'nomix_e2b_pgid="$("$nomix_e2b_ps" -o pgid= -p "$$" | "$nomix_e2b_tr" -d " ")"',
    `printf '%s\\n' "$nomix_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t nomix_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$nomix_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$nomix_e2b_env_bin" -i -- "\${nomix_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'nomix_e2b_status=$?',
    `printf '%s\\n' "$nomix_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$nomix_e2b_status"',
  ].join('\n')
  const argv = spec.argv.map(quoteE2BShellArg).join(' ')
  const bootstrap = [
    `mapfile -d '' -t nomix_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'nomix_e2b_env_bin="$(command -v env)"',
    'nomix_e2b_setsid="$(command -v setsid)"',
    'nomix_e2b_bash="$(command -v bash)"',
    'nomix_e2b_node="$(command -v node)"',
    'nomix_e2b_ps="$(command -v ps)"',
    'nomix_e2b_tr="$(command -v tr)"',
    'nomix_e2b_tee="$(command -v tee)"',
    'nomix_e2b_head="$(command -v head)"',
    'nomix_e2b_rm="$(command -v rm)"',
    'for nomix_e2b_tool in "$nomix_e2b_env_bin" "$nomix_e2b_setsid" "$nomix_e2b_bash" "$nomix_e2b_node" "$nomix_e2b_ps" "$nomix_e2b_tr" "$nomix_e2b_tee" "$nomix_e2b_head" "$nomix_e2b_rm"; do',
    '  [[ "$nomix_e2b_tool" == /* && -x "$nomix_e2b_tool" ]] || exit 125',
    'done',
    `exec "$nomix_e2b_env_bin" -i -- "\${nomix_e2b_env[@]}" "$nomix_e2b_setsid" --wait -- "$nomix_e2b_bash" -c ${quoteE2BShellArg(inner)} nomix-e2b "$nomix_e2b_env_bin" "$nomix_e2b_node" "$nomix_e2b_ps" "$nomix_e2b_tr" "$nomix_e2b_tee" "$nomix_e2b_head" "$nomix_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}

...
import { e2bControlEnvs } from '@nomix-ai/nomix-e2b'
import type { Sandbox } from '@nomix-ai/nomix-e2b'
import { SENSITIVE_ENV_PATTERN } from '@nomix-ai/nomix-subprocess'
...
export async function readRemoteEnvironment(sandbox: Sandbox, signal?: AbortSignal): Promise<string> {
  ...
  const result = await sandbox.commands.run(
    'set -o pipefail; nomix_e2b_passwd="$(getent passwd "$(id -u)")"; IFS=: read -r _ _ _ _ _ nomix_e2b_home _ <<<"$nomix_e2b_passwd"; test -n "$nomix_e2b_home" -a -d "$nomix_e2b_home"; printf \'%s\' "$nomix_e2b_home" | base64 -w 0; printf \'\\n\'; env -0 | base64 -w 0',
...
export function scrubRemoteEnvironment(raw: string): Map<string, string> {
  ...
  for (const [name, value] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('NOMIX_') || SENSITIVE_ENV_PATTERN.test(name)) continue
...
export function bootstrapEnvironment(raw: string): Record<string, string> {
  ...
  for (const [name] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('NOMIX_') || SENSITIVE_ENV_PATTERN.test(name)) environment[name] = ''
  }
  return environment
}

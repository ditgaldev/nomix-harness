 ...
 *
 * @module @nomix-ai/nomix-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `nomix-llm-replay` via `$NOMIX_SNAPSHOT_CHILD_FILES` so each child
...
export function snapshotSpillRoot(
  ...
  return `${root}/nomix-acp-snap-${key}`
}
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      NOMIX_SNAPSHOT: opts.mode,
      NOMIX_SNAPSHOT_FILE: opts.fixtureFile,
      NOMIX_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      NOMIX_SNAPSHOT_SPILL_ROOT: spillRoot,
      NOMIX_HOME: join(cwd, '.nomix'),
      NOMIX_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { NOMIX_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { NOMIX_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in nomix-llm-replay's loadSessionScripts

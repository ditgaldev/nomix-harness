 ...
 * zero-build dev path, resolving `@nomix-ai/nomix-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @nomix-ai/nomix-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'NOMIX_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.NOMIX_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated NOMIX homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { NOMIX_HOME: join(cwd, '.nomix'), NOMIX_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })

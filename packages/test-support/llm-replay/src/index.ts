...
import { decodeStorageRecord, type SessionEvent } from '@nomix-ai/nomix-session'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.NOMIX_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $NOMIX_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.NOMIX_SNAPSHOT_OVERRIDE
  const childEnv = process.env.NOMIX_SNAPSHOT_CHILD_FILES

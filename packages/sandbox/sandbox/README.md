...
Implementations: [`@nomix-ai/nomix-sandbox-local`](../sandbox-local/) (Linux: `bwrap`, else the per-platform Landlock launcher; macOS: `sandbox-exec`/Seatbelt). Consumers: [`@nomix-ai/nomix-bash-sandbox`](../../shell/bash-sandbox/) (wraps `['bash', '-c', command]`).

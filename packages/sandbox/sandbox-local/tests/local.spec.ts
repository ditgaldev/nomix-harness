...
import { LAUNCHER_FAILURE_EXIT } from '@nomix-ai/node-addon-landlock-run'
import { SANDBOX_UNAVAILABLE, SandboxUnavailableError } from '@nomix-ai/nomix-sandbox'
import type { SandboxPolicy } from '@nomix-ai/nomix-sandbox'
import {
  LocalSandboxProvider,
} from '@nomix-ai/nomix-sandbox-local'
import type { Config } from '@nomix-ai/nomix-sandbox-local'
...
function absentRunnerEntry(): string {
  return join(mkdtempSync(join(tmpdir(), 'nomix-absent-acl-entry-')), 'runner.js')
}
...
function fakeLauncher(report = 'landlock: fully enforced'): string {
  const dir = mkdtempSync(join(tmpdir(), 'nomix-fake-landlock-'))
...
function fakeSeatbeltExec(status: number): string {
  const dir = mkdtempSync(join(tmpdir(), 'nomix-fake-seatbelt-'))
...
describe('the platform chains', () => {
  ...
  // live in @nomix-ai/nomix-sandbox-windows-acl/tests/provider-chain.spec.ts
...
describe('probeTimeoutMs config', () => {
  ...
  it('bounds the default probes: a launcher slower than the configured timeout reads as unusable', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'nomix-slow-landlock-'))
    const launcher = join(dir, 'landlock-run')
...
describe('the windows-acl probe (runner invocation contract)', () => {
  ...
  it('prefers the built lib/runner.js entry when the resolved file exists', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'nomix-fake-acl-entry-'))

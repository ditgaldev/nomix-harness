...
import SessionStore, { type Session, type SessionEvent } from '@nomix-ai/nomix-session'
import InvariantRegistry, { InvariantError } from '@nomix-ai/nomix-invariants'
import * as SandboxPolicyInvariant from '@nomix-ai/nomix-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@nomix-ai/nomix-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@nomix-ai/nomix-sandbox-policy',
    })
  })
})

...
| [`bundle/`](bundle/README.md) | 可安装的 `nomix --profile` 补丁层 | 产品：稳定 API |
...
**扩展插件依赖 Service Definition，绝不依赖具体提供方。** `nomix-agent-loop` 可替换；UI、钩子和工具插件使用 `nomix-agent`。包括 `nomix-agent-spine-demo` 在内的组合包可以依赖主干插件。能力会将需要独立演进的 Service Definition／Service Provider／Consumer 角色分离；详见[能力 seam](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)。

...
import { assembleContextFor, type Agent } from '@nomix-ai/nomix-agent'
...
import type { SubagentStartRequest } from '@nomix-ai/nomix-subagent'
...
/**
 * Drives the REAL plugin body: mounts `nomix-tool-subagent` on a real
...
describe('nomix-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @nomix-ai/nomix-jobs')
  })
...
describe('nomix-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'nomix-tool-subagent-continuable-'))
    roots.push(root)

...
describe('task admission and package contracts', () => {
  it('ships one independently installable provider-only Bundle patch', () => {
    ...
    expect(manifest.dependencies).not.toHaveProperty('@nomix-ai/nomix-subagent-codex')
    ...
    const parsed = yaml.load(readFileSync(resolve(root, manifest.nomix!.bundle!.patch!), 'utf8'))
  ...
  it('keeps named instances, runs, and HMR ownership isolated', async () => {
    ...
    vi.spyOn(ctx.subprocess, 'spawn').mockImplementation((spec) => {
      ...
      return spec.env?.NOMIX_CLAUDE_INSTANCE === 'safe'
        ? safeChild.handle
        : bypassChild.handle
    })
    ...
    expect(queryOptions.map(options => ({
      instance: options.env?.NOMIX_CLAUDE_INSTANCE,
    ...
    expect(spawnSpecs.map(spec => ({
      instance: spec.env?.NOMIX_CLAUDE_INSTANCE,

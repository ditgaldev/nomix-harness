...
import { resolveExampleLaunch, runLoaderSmoke } from '@nomix-ai/nomix-loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/subagent/subagent-nomix-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'nomix-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'nomix-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        NOMIX_TEST_CHILD_COMMAND: childLaunch.command,
        NOMIX_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        NOMIX_TEST_CHILD_ENV: JSON.stringify({

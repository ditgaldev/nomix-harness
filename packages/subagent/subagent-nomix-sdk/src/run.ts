/**
 * Fresh-process SDK subagent client. Drives one child Nomix Harness
 * runtime over stdio JSON-RPC through `@nomix-ai/nomix-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@nomix-ai/nomix-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @nomix-ai/nomix-subagent-nomix-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { NomixHarness, type HarnessNotification } from '@nomix-ai/nomix-sdk-client'
import type { ContentBlock } from '@nomix-ai/nomix-llm'
import { SessionId, type SessionEvent, type TurnEndReason } from '@nomix-ai/nomix-session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@nomix-ai/nomix-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@nomix-ai/nomix-subagent'
...
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `nomix-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `NOMIX_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `NOMIX_*` fact survives while ambient namesakes never leak.
   */

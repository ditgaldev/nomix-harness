...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@nomix-ai/nomix-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@nomix-ai/nomix-hook-protocol'
import type { RunHookOptions } from '@nomix-ai/nomix-hook-protocol'
 ...
 * executor (nomix-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what nomix-bash-local's resolve does for the fields runHook sets.
      return {

# @nomix-ai/nomix-hook-protocol

English | [中文](README.zh.md)

The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@nomix-ai/nomix-hooks-claude-code`, `@nomix-ai/nomix-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`nomix-hook-protocol`) | The bridge (`nomix-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `nomix-hooks-claude-code` and `nomix-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.

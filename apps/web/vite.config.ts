...
const DEFAULT_CLIENT_TITLE = 'NOMIX Local Build'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.NOMIX_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'nomix-client-document-title',
    transformIndexHtml(html) {
      return html.replace('<title>NOMIX Local Build</title>', `<title>${title}</title>`)
    },
  }
}

...
interface ListMetrics {
  ...
  hoverRules: string[]
  /** `--nomix-scrollbar-thumb` resolved on the list, serialized as a colour. */
  token: string
  /** `--nomix-scrollbar-thumb-hover` resolved on the list, serialized the same way. */
  hoverToken: string
...
function measureList(page: Page): Promise<ListMetrics> {
  return page.evaluate(() => {
    ...
    return {
      ...
      hoverRules,
      token: resolve('--nomix-scrollbar-thumb'),
      hoverToken: resolve('--nomix-scrollbar-thumb-hover'),
...
interface PaletteMetrics {
  ...
  hovered: ListMetrics
  /** `--nomix-scrollbar-thumb` with the pointer parked outside the column. */
  quietThumb: string
}
...
function renderGeometry(light: PaletteMetrics, dark: PaletteMetrics): string {
  const palette = (name: string, { hovered: metrics, quietThumb }: PaletteMetrics): string[] => [
    `## ${name}`,
    '',
    `- --nomix-scrollbar-thumb, pointer outside the sidebar: ${quietThumb}`,
    ...
    `- --nomix-scrollbar-thumb, pointer over the list: ${metrics.token}`,
    `- --nomix-scrollbar-thumb-hover, pointer over the list: ${metrics.hoverToken}`,
...
/**
 * Resolve `--nomix-scrollbar-thumb` as the list sees it, without the rest of the
...
function resolveThumb(page: Page): Promise<string> {
  return page.evaluate(() => {
    ...
    if (list === null) throw new Error('sidebar session list not in the DOM')
    const probe = document.createElement('span')
    probe.style.color = 'var(--nomix-scrollbar-thumb)'
    list.append(probe)
    const value = getComputedStyle(probe).color
    probe.remove()
    return value
...
describe('web e2e: sidebar session list scrollbar (reserved gutter / themed thumb)', () => {
  ...
  it('renders the themed thumb through the WebKit path in both palettes', async () => {
    ...
    expect(light.hoverRules).toEqual(['var(--nomix-scrollbar-thumb-hover)'])

...
import type { ReplayEntry, ReplayOverrideDoc } from '@nomix-ai/nomix-llm-replay'
...
import type {} from '@nomix-ai/nomix-session-title'
import {
  launchWebScaffold,
  seedSession,
  watchConsole,
...
async function startMutationProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Reflect.set(globalThis, '__nomixPerfMutationProbe', probe)
  })
}

async function stopMutationProbe(page: Page): Promise<MutationProbeResult> {
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__nomixPerfMutationProbe') as
    ...
    Reflect.deleteProperty(globalThis, '__nomixPerfMutationProbe')
...
async function startUserRenderProbe(
  ...
  await page.evaluate((expectedMarker) => {
    ...
    Reflect.set(globalThis, '__nomixPerfUserRenderProbe', probe)
  }, marker)
  await send.evaluate((button) => {
    button.addEventListener('click', (event) => {
      const probe = Reflect.get(globalThis, '__nomixPerfUserRenderProbe') as
...
async function stopUserRenderProbe(
  ...
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__nomixPerfUserRenderProbe') as
      ...
      | undefined
    Reflect.deleteProperty(globalThis, '__nomixPerfUserRenderProbe')
    if (
...
async function launchPerformanceWorld(
  ...
  try {
    if (options.replay === undefined) {
      ...
      replayDir = await mkdtemp(join(tmpdir(), 'nomix-web-perf-replay-'))

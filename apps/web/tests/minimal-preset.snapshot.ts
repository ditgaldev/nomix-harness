...
import type { AgentHandle } from '@nomix-ai/nomix-agent'
import { CallId, createUserMessage } from '@nomix-ai/nomix-llm'
...
describe('minimal agent preset', () => {
  ...
  it('sends the exact RL prompt and schemas, then executes the persistent shell and editor', async () => {
    ...
    expect(agentHandle.agent.session.events.some(event => event.type === 'user/message'
      ...
      && event.data.source.plugin === '@nomix-ai/nomix-system-prompt')).toBe(false)
    ...
    await scaffold.ctx.tools.execute({
      ...
      name: 'bash',
      arguments: { command: `cd ${JSON.stringify(stateDir)} && export NOMIX_MINIMAL_STATE=PERSISTED` },

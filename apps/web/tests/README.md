...
the deliberate composition divergences from `nomix web` — are documented in
...
## Do not import `@nomix-ai/nomix-client-*` here
...
`@nomix-ai/nomix-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@nomix-ai/nomix-client-web` and the boot-manifest
type from `@nomix-ai/nomix-client-modules/client`: booting the real shell is what
...
`@nomix-ai/nomix-client-runtime/client` because `client/runtime` is reachable

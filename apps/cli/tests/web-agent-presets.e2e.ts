...
import { boot, healProfilesModuleFallback, loadOverlayPatches } from '@nomix-ai/nomix-app-boot'
...
async function bootWeb(
  ...
  return await boot('nomix-test', rootConfig, patches, (bootCtx) => {
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@nomix-ai/nomix-subagent-codex' },
        { id: 'subagent-claude-code', name: '@nomix-ai/nomix-subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {

...
const binScript = fileURLToPath(new URL('./fixtures/nomix-badge/snapshot.ts', import.meta.url))
const configPath = fileURLToPath(new URL('./fixtures/nomix-badge/cordis.yml', import.meta.url))
const defaultConfigPath = fileURLToPath(new URL('./fixtures/nomix-badge/default.cordis.yml', import.meta.url))
const tsconfigPath = fileURLToPath(new URL('../../../tsconfig.json', import.meta.url))
const badgeAssetsPath = fileURLToPath(new URL('../../../packages/skill/skill-badge/assets/', import.meta.url))

describe('nomix badge assembled snapshot', () => {
  it('advertises and loads the opt-in bundled skill through the shipped app', async () => {
    const disabled = await runLoaderSmoke({
      label: 'disabled nomix badge skill snapshot',
      tempDirPrefix: 'headless-snapshot-nomix-badge-disabled-',
      binScript,
    ...
    const enabled = await runLoaderSmoke({
      label: 'nomix badge skill snapshot',
      tempDirPrefix: 'headless-snapshot-nomix-badge-',
      binScript,
      libBinScript: binScript,
      configPath,
      tsconfigPath,
    ...
    expect(disabledSnapshot).toMatchInlineSnapshot(`
      {
        ...
        "result": {
          "content": [
            {
              "text": "Error: skill "nomix-badge" is unknown or no longer available",
              "type": "text",
            },
          ],
          "error": {
            "message": "skill "nomix-badge" is unknown or no longer available",
          },
          "isError": true,
        },
        "summary": null,
    ...
    expect(enabledSnapshot).toMatchInlineSnapshot(`
      ...
      A user may also invoke a skill directly; its <skill_content> block then appears in this conversation. Follow it, and do not call the \`skill\` tool again for that skill.
        ...
        "result": {
          "content": [
            {
              "text": "<skill_content name="nomix-badge">
      <skill_resources>

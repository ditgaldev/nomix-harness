 ...
 * breaks this gate instead of every developer's `pnpm nomix`; the built-bin
...
describe('nomix source launcher (node --import tsx/esm)', () => {
  it('launches the source CLI without building', async () => {
    ...
    expect(rootPackage.scripts?.nomix).toBe('node --import tsx/esm apps/cli/src/bin.ts')
  })

  it('boots the source entry and requires a profile', async () => {
    const result = await execa(process.execPath, ['--import', 'tsx/esm', nomixSourceBin], {
      cwd: repoRoot,
      input: '',
      timeout: 25_000,
      killSignal: 'SIGKILL',
    ...
    if (result.timedOut) {
      throw new Error(`nomix source launch did not exit within 25s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }

...
import { parseNomixArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseNomixArgs(argv, '1.2.3')
...
describe('parseNomixArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@nomix-ai/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@nomix-ai/cordis'] })

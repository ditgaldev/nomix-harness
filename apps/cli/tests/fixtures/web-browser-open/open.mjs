...
export default async function open(url) {
  ...
  console.log(`nomix browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__NOMIX_BOOT__'),
    apiKeyPresent: process.env.DEEPSEEK_API_KEY !== undefined,
    nomixHomePresent: process.env.NOMIX_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.nomix-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}

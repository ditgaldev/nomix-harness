...
const exitMarker = join(process.cwd(), `.nomix-browser-open-${process.pid}`)
...
if (process.env.NOMIX_BROWSER_OPEN_TEST_EXIT_ON_READY === '1') {
  const originalLog = console.log
  console.log = (...args) => {
    originalLog(...args)
    if (typeof args[0] === 'string' && args[0].startsWith('nomix web: ')) {
      setTimeout(() => process.exit(0), 250)
    }
  }
}

if (process.env.NOMIX_BROWSER_OPEN_TEST_EXIT_ON_FAILURE === '1') {

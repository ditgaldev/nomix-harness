 ...
 * the REAL shipped bundle layers (nomix-base + nomix-web-app resolved from the
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL nomix-base/nomix-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'nomix-windows-home-'))

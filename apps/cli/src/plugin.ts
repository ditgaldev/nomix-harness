 ...
 * `nomix.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `nomix.bundle` joins the layer stack; a
 ...
 * gained its `nomix.bundle` declaration in a newer version.
 ...
 * @returns true when the package manifest declares `nomix.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.nomix?.bundle?.patch !== undefined
}

/**
 * Reconcile `nomix.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `nomix.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.nomix?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no nomix.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (nomix-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.nomix = { ...after.nomix, profile: { ...after.nomix?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `nomix` was invoked from.

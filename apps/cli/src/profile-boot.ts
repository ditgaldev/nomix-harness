/**
 * Shared profile boot for every `nomix` surface: resolve the profile, stack its
 * patch layers (bundle layers in `nomix.profile.bundles` order, the profile's
 ...
 * @module @nomix-ai/nomix-harness/profile-boot
 */

import { writeFileSync } from 'node:fs'
import { join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'
import { FiberState, type Context } from '@nomix-ai/cordis'
...
/**
 * The home-level user patch layer (`$NOMIX_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$NOMIX_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveNomixHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this nomix installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the NOMIX_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# nomix profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's nomix.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `NOMIX_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$NOMIX_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `nomix.profile.bundles` order (the base bundle gates the shell stacks by
 * platform on its own rows), the profile's user layer, the home-level user
 * layer (`$NOMIX_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  // The writable root the roster appends is `nomix-agent-presets`' own, so a
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.NOMIX_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@nomix-ai/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@nomix-ai/cordis-plugin-hmr', config: { root: [] } })
      }

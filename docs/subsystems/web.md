...
The model-facing tool argument is just a `query`; `maxResults` is a consumer-owned bound (`nomix-tool-web`'s `searchMaxResults` config, default `8`) passed through the seam and enforced on the way back — if a provider over-returns, the seam truncates `sources[]` and sets `truncated`.

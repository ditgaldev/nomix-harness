...
The neutral vocabulary lives in `nomix-tools`; tools never import a UI or transport type. Host/client runtimes map each `card` into their own view. The design and the why are in [the render-intent-union Agent Note](../../.agents/notes/implemented/architecture/2026-07-02-tool-render-intent-union.md); `nomix-tool-fs` (generic/diff) and `nomix-tool-bash` (terminal) are the reference implementations.

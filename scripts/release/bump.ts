 ...
 *
 * The nomix family shares one version across its publishable members, private
...
/** One private nomix package whose version follows the publishable family. */
interface PrivateNomixVersion {
  /** Repository-relative manifest path. */
  readonly manifestPath: string
...
/**
 * Discover private package manifests that share the nomix version without joining
 * its publish set.
 * @param root - repository root.
 * @returns Private package manifests sorted by path.
 */
function privateNomixVersions(root: string): PrivateNomixVersion[] {
...
/**
 * Plan the nomix family's rewrite: one version for every publishable member,
...
export function planShared(
  ...
  for (const entry of privateNomixVersions(root)) {

...
const SESSION_PACKAGE = '@nomix-ai/nomix-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@nomix-ai/nomix-session/types'
 ...
 * top-level declaration (in `@nomix-ai/nomix-session`) and any declaration
 * merge inside a `declare module '@nomix-ai/nomix-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @nomix-ai/nomix-session/known-event-types',
    ' */',
    '',
    '/**',

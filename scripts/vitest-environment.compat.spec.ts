...
describe('Vitest jsdom compatibility', () => {
  it('provides isolated browser storage instead of Node process storage', () => {
    ...
    localStorage.setItem('nomix-vitest-storage-probe', 'available')

    expect(localStorage.getItem('nomix-vitest-storage-probe')).toBe('available')
    localStorage.removeItem('nomix-vitest-storage-probe')
  })
})

...
describe('client bundle purity gate', () => {
  ...
  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@nomix-ai/nomix-host-apiproxy/api')).toBeNull()
    expect(resolveId('@nomix-ai/nomix-session/surface')).toBeNull()
    expect(resolveId('@nomix-ai/nomix-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@nomix-ai/nomix-goal/remote')).toBeNull()
    expect(() => resolveId('@nomix-ai/nomix-goal')).toThrow(/purity/)
    expect(() => resolveId('@nomix-ai/nomix-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@nomix-ai/nomix-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @nomix-ai leak', () => {
    expect(() => resolveId('@nomix-ai/nomix-agent')).toThrow(/purity/)
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@nomix-ai/nomix-client-ui-goal')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@nomix-ai/nomix-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@nomix-ai/nomix-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')

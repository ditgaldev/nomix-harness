...
const CLIENT_BUILD_ENV_PREFIX = 'NOMIX_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'NOMIX_BUILD_CLIENT_PROFILE'

/** Public client environment required by official NOMIX artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'NOMIX_CLIENT_COMMIT_HASH'

/** Repository-relative path of the complete client build record. */
export const CLIENT_BUILD_RECORD_PATH = '.nomix-build/client-build-environment.json'
...
): Readonly<Record<`NOMIX_CLIENT_${string}`, string>> {
  return {
    NOMIX_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
 ...
 * @returns defined `NOMIX_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return { NOMIX_CLIENT_COMMIT_HASH: commitHash, ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`NOMIX_CLIENT_${string}`, string>>,
): void {

...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@nomix-ai/nomix-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@nomix-ai/nomix-a/subpath'",
      "declare module '@nomix-ai/nomix-client-ui-slots' {}",
      "const load = () => import('@nomix-ai/nomix-b')",
...
describe('package modes', () => {
  ...
  it('requires seeded workspace packages to use staticLinked and preloads to name dynamic rows', () => {
    ...
    expect(found.join('\n')).toContain('has no dynamic nomix.client row')
  })
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@nomix-ai/nomix-client-runtime/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('dependency sections', () => {
  it('accepts dynamic peer plus dev relationships, static dev inputs, and private dependencies', () => {
    ...
    const runtime = pkg('runtime', {
      inject: ['@nomix-ai/nomix-client-feature'],
      sourceUses: {
        '@nomix-ai/nomix-agent': ['packages/client/runtime/src/index.ts'],
        '@nomix-ai/nomix-client-ui-slots': ['packages/client/runtime/src/client/slots.ts'],
      ...
      peerDependencies: {
        ...
        '@nomix-ai/nomix-client-feature': 'workspace:^',
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@nomix-ai/nomix-agent': 'workspace:^',
        '@nomix-ai/nomix-client-feature': 'workspace:^',
        '@nomix-ai/nomix-client-ui-slots': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('rejects internal dependencies, static peers, and mismatched peer development ranges', () => {
    ...
    const subject = pkg('feature', {
      sourceUses: {
        '@nomix-ai/nomix-agent': ['packages/client/feature/src/index.ts'],
        [slots.name]: ['packages/client/feature/src/view.tsx'],
      },
      dependencies: { '@nomix-ai/nomix-agent': 'workspace:^' },
    ...
    expect(found).toHaveLength(2)
    expect(found.join('\n')).toContain('peer-installed NOMIX relationship')
    expect(found.join('\n')).toContain('static client input')
  })

  it('requires every peer to have the same development range', () => {
    const subject = pkg('feature', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@nomix-ai/cordis-plugin-loader': 'workspace:^' },
    })
    expect(collectClientPackageViolations(facts([subject]))).toEqual([
      'packages/client/feature/package.json: peerDependencies.@nomix-ai/cordis-plugin-loader'
      + ' is workspace:^, so devDependencies.@nomix-ai/cordis-plugin-loader must use the same range;'
  ...
  it('keeps the web shell runtime inputs development-only', () => {
    const web = pkg('web', {
      ...
      staticLinked: true,
      runtimeSourceUses: {
        '@nomix-ai/cordis-plugin-loader': ['packages/client/web/src/boot.ts'],
  ...
  it('allows npm dependency cycles', () => {
    const a = pkg('a', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@nomix-ai/nomix-client-b': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@nomix-ai/nomix-client-b': 'workspace:^' },
    })
    const b = pkg('b', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@nomix-ai/nomix-client-a': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@nomix-ai/nomix-client-a': 'workspace:^' },
    })
...
describe('module requests', () => {
  it('accepts a dynamic row supplier and its client subpath', () => {
    const ui = declaration('ui', { external: ['@nomix-ai/nomix-client-slots/client'] })
  ...
  it('rejects an explicit baseline request', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      ui.manifest + ': nomix.client.external repeats baseline module "react"; remove the explicit declaration',
    ])
  })

  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@nomix-ai/nomix-client-ui', '@nomix-ai/nomix-missing', '@nomix-ai/nomix-missing'],
      inject: ['', '@nomix-ai/nomix-a', '@nomix-ai/nomix-a'],
    })
    const found = collectClientPackageViolations(facts([], { declarations: [ui] }))
    expect(found).toHaveLength(6)
    expect(found.join('\n')).toContain('nomix.client.external contains an empty value')
    expect(found.join('\n')).toContain('nomix.client.inject contains an empty value')
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = declaration('a', {
      external: ['@nomix-ai/nomix-client-b'],
      inject: ['@nomix-ai/nomix-client-b'],
    })
    const b = declaration('b', {
      external: ['@nomix-ai/nomix-client-a'],
      inject: ['@nomix-ai/nomix-client-a'],
    })
    const found = collectClientPackageViolations(facts([], { declarations: [a, b] }))
    expect(found).toHaveLength(1)
    expect(found[0]).toContain('synchronous nomix.client.external cycle')
  })
})
...
describe('manifest declarations', () => {
  it('reports malformed arrays without hiding other packages', () => {
    ...
    const files: Record<string, unknown> = {
      'packages/g/a/package.json': {
        name: '@f/a', nomix: { client: { external: 'react', inject: ['@f/b', 1] } },
      },
      'packages/g/b/package.json': { name: '@f/b', nomix: { client: {} } },
    }
    ...
    expect(result.malformed).toEqual([
      'packages/g/a/package.json: @f/a nomix.client.external must be a string array',
      'packages/g/a/package.json: @f/a nomix.client.inject must be a string array',
    ])
  })
  ...
  it('fixes unambiguous dependency sections and declaration entries', () => {
    ...
    roots.push(root)
    const subject = pkg('feature', {
      external: ['', 'react', '@nomix-ai/nomix-client-feature', '@nomix-ai/nomix-missing'],
      inject: ['', '@nomix-ai/nomix-agent', '@nomix-ai/nomix-agent'],
      sourceUses: {
        '@nomix-ai/nomix-agent': ['packages/client/feature/src/index.ts'],
        '@nomix-ai/nomix-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@nomix-ai/nomix-agent': 'workspace:*',
    ...
    const manifest = {
      name: subject.name,
      nomix: { client: { external: subject.external, inject: subject.inject, platform: 'web' } },
    ...
    const fixed = JSON.parse(readFileSync(join(root, subject.manifest), 'utf8')) as {
      nomix: { client: { external: string[]; inject: string[] } }
      dependencies?: Record<string, string>
      peerDependencies: Record<string, string>
      devDependencies: Record<string, string>
    }
    expect(fixed.nomix.client).toMatchObject({
      external: ['@nomix-ai/nomix-missing'],
      inject: ['@nomix-ai/nomix-agent'],
    })
